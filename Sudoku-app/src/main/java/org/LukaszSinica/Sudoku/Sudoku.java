...
public class Sudoku {
	...
	public static boolean checkAnswer(Integer row, Integer col, Integer tileAnswer, Integer[][] board,
			Integer[][] playerBoard) {
				...
				if (board[i][j] == 0) {
					...
					for (int value : sudokuNumbers) {
						...
						if (!checkRow(board[i], value) && !checkCol(board, value, j)) {
							...
							if (!checkRow(square[0], value) && !checkRow(square[1], value)
									&& !checkRow(square[2], value)) {
								board[i][j] = value;
